    ...
    def run(self):
        try:
            ...
            for json_file in self.json_files:
                self.status_update.emit(f"Processing file: {os.path.basename(json_file)}")
                data, image_urls = self.extract_image_urls(json_file)
                url_mapping = {}

                with tempfile.TemporaryDirectory() as temp_dir:
                    for i, url in enumerate(image_urls):
                        self.status_update.emit(f"Processing image {i+1} of {len(image_urls)}")
                        local_path = self.download_image(url, temp_dir)
                        if local_path:
                            file_name = os.path.basename(local_path)
                            remote_path = f'migrated_images/{file_name}'
                            firebase_url = self.upload_to_firebase(local_path, remote_path)
                ...
                # Save updated JSON
                output_file = os.path.join(os.path.dirname(json_file), 'updated_' + os.path.basename(json_file))
        ...
        for item in data:
            if isinstance(item, str):
                ...
                for url in img_tags:
                    if url.startswith('//'):
                        url = 'https:' + url
                    if 'storage.googleapis.com' in url and 'cdn.testbook.com' not in url:
...
class App(QWidget):
    ...
    def selectJsonFiles(self):
        filenames, _ = QFileDialog.getOpenFileNames(self, "Select JSON Files", "", "JSON Files (*.json)")
        if filenames:
    ...
    def selectFirebaseConfig(self):
        filename, _ = QFileDialog.getOpenFileName(self, "Select Firebase Config File", "", "JSON Files (*.json)")
        if filename:
            self.firebase_label.setText(f'Firebase Config: {os.path.basename(filename)}')
            self.firebase_config_file = filename

    def startMigration(self):
        if self.json_files and hasattr(self, 'firebase_config_file'):
            self.migration_thread = ImageMigrationThread(self.json_files, self.firebase_config_file)
            ...
            QMessageBox.warning(self, "Input Error", "Please select JSON files and Firebase config file.")
    ...
    def migrationFinished(self):
        ...
        QMessageBox.information(self, "Migration Complete", "Image migration for all files has been completed successfully!")
